...
export async function POST(req: NextRequest) {
  ...
  if (action === 'mulai') {
    const existing = await prisma.sesi.findFirst({ where: { mejaId, status: 'AKTIF' } })
    if (existing) return NextResponse.json({ error: 'Meja sedang dipakai' }, { status: 400 })
    const sesi = await prisma.sesi.create({
      data: { mejaId, pelanggan, memberId: memberId || null, tarif, status: 'AKTIF' }
    })
  ...
  if (action === 'reserve') {
    const sesi = await prisma.sesi.create({
      data: { mejaId, pelanggan, tarif, status: 'RESERVED' }
    })
    return NextResponse.json(sesi)
  }
  ...
  if (action === 'checkout' && sesiId) {
    ...
    await prisma.transaksi.create({
      data: { sesiId, userId: user.id, jenis: 'SEWA', total: biaya }
    })
    if (sesi.memberId) {
      const poinBaru = Math.floor(biaya / 1000)
      await prisma.member.update({
        where: { id: sesi.memberId },
  ...
  if (action === 'batal-reserve' && sesiId) {
    await prisma.sesi.update({ where: { id: sesiId }, data: { status: 'SELESAI' } })
